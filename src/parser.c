...
parser_t *
parser_make (token_vector_t *tokens)
{
...
function_node_t *
parser_find_function (parser_t *p, char *name)
{
...
vardec_node_t *
parser_find_var (parser_t *p, char *name)
{
...
parser_parse_statement (parser_t *p)
{
        ...
        if (tk->type == TOKEN_KEYWORD && str_equals (tk->text, "return"))
        {
                ...
                {
                        rn->return_n.s
                            = parser_consume (p, TOKEN_STRING)->text;
...
void
parser_fn_validate_params (node_t *fn, node_param_vector_t *params,
                           token_t *tk)
{
...
void
parser_error (token_t *tk, char *fmt, ...)
{
        va_list args;
        va_start (args, fmt);
