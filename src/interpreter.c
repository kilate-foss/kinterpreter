...
static interpreter_result_t _interpreter_run_fn(interpreter_t *self,
                                                node_t *func,
...
interpreter_result_t interpreter_run(interpreter_t *self)
{
        ...
        node_t **mainPtr =
            (node_t **)hm_get(self->functions, MAIN_FUNCTION_NAME);
        if (mainPtr == NULL) {
        ...
        if (main->function_n.return_type == NULL ||
            !str_equals(main->function_n.return_type, MAIN_FUNCTION_RETURN)) {
                error_fatal(MAIN_FUNCTION_NAME " function should return " MAIN_FUNCTION_RETURN);
        }

        return interpreter_run_fn(self, main, NULL);
}

interpreter_result_t interpreter_run_fn(interpreter_t *self, node_t *fn,
