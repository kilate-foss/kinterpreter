...
const void *
vector_get (vector_t *self, size_t index)
{
        if (self == NULL)
                error_fatal ("Vector is null");
        if (index >= self->size)
...
vector_reserve (vector_t *self, const size_t size)
{
        ...
        if (self->capacity <= size)
        {
                ...
                memset ((char *)self->data + self->capacity * self->itemSize,
                        0, (size - self->capacity) * self->itemSize);
                self->capacity = size;
        }
}

void
vector_set (vector_t *self, size_t index, const void *item)
{
        if (self == NULL)
                error_fatal ("Vector is null");
        if (index >= self->size)
...
void
vector_remove (vector_t *self, size_t index)
{
        if (self == NULL)
                error_fatal ("Vector is null");
        if (index >= self->size)
